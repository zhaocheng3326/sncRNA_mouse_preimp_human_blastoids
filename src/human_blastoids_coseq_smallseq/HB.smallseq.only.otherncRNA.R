#' ---
#' title: "other ncRNA UMAP for blastoids small"
#' output:
#'  html_document:
#'    code_folding: hide
#' ---
...
meta.filter <-  readRDS(paste0("tmp_data/",TD,"/HB.small.ac_miRNA.data.ob.umap.rds")) %>% filter(batch=="HB.smallseq")  %>% select(cell,embryo,devTime,SC,EML,RNA_EML,small_EML)
...
#' type of ncRNA
sel.type <- c("miRNA","tRNA","snoRNA","piRNA","snRNA",'rRNA') 
data.ob.list <- list()
...
if (file.exists(paste0("tmp_data/",TD,"/HB.coseq.small.otherDE.rds"))) {
  data.ob.list <- readRDS(paste0("tmp_data/",TD,"/HB.smallseq.only.otherncRNA.ob.rds"))
  DE.ncRNA.list <- readRDS(paste0("tmp_data/",TD,"/HB.smallseq.only.otherDE.rds"))
  data.ob.umap.list <- readRDS(paste0("tmp_data/",TD,"/HB.smallseq.only.otherncRNA.umap.rds"))
}else{
  for (s in sel.type) { ## snoRNA also is a marker
    ...
    temp.M <- temp.M[colSums(temp.counts>0) > 10,]
    ...
    if (s=="piRNA") {
      data.temp <- data.temp %>% FindVariableFeatures(verbose=F,nfeatures=2000) %>% ScaleData(verbose = FALSE) %>% RunPCA( verbose = FALSE) %>% RunUMAP( reduction = "pca", dims = 1:npc,verbose = FALSE) %>% FindNeighbors(reduction = "pca", dims = 1:npc,verbose = FALSE,k.param=10)
    }else if (s=="rRNA") {
      ...
      data.temp <- data.temp %>% FindVariableFeatures(verbose=F,nfeatures=250) %>% ScaleData(verbose = FALSE) %>% RunPCA( verbose = FALSE) %>% RunUMAP( reduction = "pca", dims = 1:npc,verbose = FALSE) %>% FindNeighbors(reduction = "pca", dims = 1:npc,verbose = FALSE,k.param=10)
    }
  ...
  #' saving object
  saveRDS(data.ob.list ,paste0("tmp_data/",TD,"/HB.smallseq.only.otherncRNA.ob.rds"))
  saveRDS(DE.ncRNA.list ,paste0("tmp_data/",TD,"/HB.smallseq.only.otherDE.rds"))
  saveRDS(data.ob.umap.list ,paste0("tmp_data/",TD,"/HB.smallseq.only.otherncRNA.umap.rds"))
}
