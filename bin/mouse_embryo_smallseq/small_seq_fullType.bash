...
WD=$TMPD/Merged_Data/${SA}
cutada=$BDOC/sc_smallRNA_annotation/Mouse/cutadapt_3prime.fa ### From paper
RGB=$BDOC/sc_smallRNA_annotation/Mouse/RefSeq/clean_chr_refdata/bowtie/bowtie
RGH=$BDOC/sc_smallRNA_annotation/Mouse/RefSeq/clean_chr_refdata/hisat/genome.trans
REF=$BDOC/sc_smallRNA_annotation/Mouse/RefSeq/clean_chr_refdata/fasta/genome.fa

BREF=$BDOC/sc_smallRNA_annotation/Mouse/RefSeq/clean_chr_refdata/bowtie/bowtie
HsaM=$BDOC/sc_smallRNA_annotation/Mouse/mirBase/mmu.mature.fa
#NHsaM=$BDOC/sc_smallRNA_annotation/Mouse/mirBase/non.mmu.mature.fa
HsaH=$BDOC/sc_smallRNA_annotation/Mouse/mirBase/mmu.hairpin.fa
Hsa_GTF=$BDOC/sc_smallRNA_annotation/Mouse/mirBase/mmu.gff3
...
#cutadapt -a file:${cutada}   -e 0.1 -O 1 -m 18  -u 2 -o ${SA}.extract.cutadp.fastq ${SA}.extract.fastq  >${SA}.extract.cutadp.log  ## trim 2bp from begining, overlap at least 10bp 
rm ${SA}.fastq ${SA}.extract.fastq
#rm  -f ${SA}.extract.fastq
...
seqkit fx2tab aligned.T3.0bp.fastq|cut -f 1| cut -f 1 -d " " > aligned.T3.0bp.ID 
...
do
   ...
   if [ -e aligned.T3.${i}bp.fastq ];then
   	seqkit fx2tab aligned.T3.${i}bp.fastq|cut -f 1| cut -f 1 -d " " > aligned.T3.${i}bp.ID
   fi
...
for i in T3.0bp T3.1bp T3.2bp T3.3bp T5.1bp T5.2bp T5.3bp;do cat aligned.${i}.ID| awk -F "\t" -v a=${i} '{print $1"\t"a}' >> aligned.fastq.ID;done
...
do
	intersectBed -a temp -b <(awk -v a=${i} -F "\t" '{if ($8==a) print $0}' $BDOC/sc_smallRNA_annotation/Mouse/small.anno.bed) -s -wa -wb >> temp.aligned.sort.dedup.reMap.bam.ov.small.detail
	intersectBed -a temp -b <(awk -v a=${i} -F "\t" '{if ($8==a) print $0}' $BDOC/sc_smallRNA_annotation/Mouse/small.anno.bed) -s -v > temp.remain
...
########## feature counting
#### Get counts
python $SRC/count_smallRNA.detailed.ordered.exp.withLen.py -l $BDOC/sc_smallRNA_annotation/Mouse/reads.report.order -s $BDOC/sc_smallRNA_annotation/Mouse/reads.report.order  -e temp.aligned.sort.dedup.reMap.bam.ov.small.detail -o aligned.detailed
